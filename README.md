...
- **AI 助手**（需 DeepSeek key）：
  ...
  搜索结果缓存 10 分钟（`cache.py`），摘要/问答基于缓存上下文；这是缓存
  TTL，不是单次搜索的执行预算。
- **旧文找回（多通道）**：针对旧文章沉在栏目列表几十页之后的问题——
  ...
  - **动态栏目发现补充**：渲染搜索会优先少量抓取动态栏目/列表入口，并从其
    分页中取回文章正文；不受所选深度 1～3 限制，但仍与静态抓取、渲染抓取
    和其他结构化发现通道共享整体 120 秒截止时间与统一页面预算。
...
- 政府/门户类动态站：自动模式通常能自行识别并补搜；已知是动态站可选
  「强制 JS 渲染」省掉静态那一趟。
- 从首页搜很深的旧内容（如多年前的通知）：可选深度 2~3 + 渲染；已知
  栏目页地址时直接填栏目页 + 深度 1 通常更快。
- 所有静态抓取、渲染抓取和结构化发现共享整体 120 秒截止时间与统一页面
  预算；达到截止时间时返回已有结果，并在诊断中标记 `partial: true`。
