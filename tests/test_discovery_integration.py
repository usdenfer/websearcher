...
def test_body_only_keyword_is_found_independent_of_bfs_depth(
    ...
    for depth in (1, 2, 3):
        ...
        assert all(KEYWORD not in item["pageTitle"] for item in data["results"])
        assert all(KEYWORD not in item["pageUrl"] for item in data["results"])
...
def test_failed_api_falls_back_to_category(discovery_site, monkeypatch):
    ...
    response = _search(TestClient(app), discovery_site, depth=1)
